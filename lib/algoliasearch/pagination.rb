  ...
  module Pagination
    ...
    def self.create(results, total_hits, options = {})
      ...
      begin
        ...
        backend = AlgoliaSearch.pagination_backend.to_s.gsub(/\/(.?)/) { "::#{$1.upcase}" }.gsub(/(?:^|_)(.)/) { $1.upcase }
        Object.const_get(:AlgoliaSearch).const_get(:Pagination).const_get(backend).create(results, total_hits, options)
      rescue NameError
